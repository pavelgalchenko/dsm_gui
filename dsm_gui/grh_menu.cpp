...
void GRH_Menu::set_validators() {
    ...
    ui->targetType->addItems(dsm_gui_lib::sortStringList(hosttarget_inputs.values()));
...
void GRH_Menu::apply_data() {
    ...
    for (int line_num=1; line_num<grh_data.length(); line_num++) {
        ...
        line_items = grh_data[line_num-1].remove("\"").split(QRegExp("\\s"), Qt::SkipEmptyParts);
        switch (line_num) {
        case 1: // File header
            break;
        case 2:
...
void GRH_Menu::on_savedefaultButton_clicked() {
    int response =dsm_gui_lib::warning_message("Overwrite Default Graphics file?");
    if (response == QMessageBox::Ok) {
