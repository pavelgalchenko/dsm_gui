...
void NOS_Menu::apply_data() {
   ...
   line_num = 2;
   line_items =
       nos_data[line_num - 1].split(QRegExp("\\s"), QString::SkipEmptyParts);
...
void NOS_Menu::on_loaddefaultButton_clicked() {
   ...
   if (response == QMessageBox::Ok) {
      ...
      QFile::copy(inout_path + "__default__/Inp_NOS3.txt",
                  inout_path + "Inp_NOS3.txt");
      receive_data();
      apply_data();
   } else {
      return;
...
void NOS_Menu::on_savedefaultButton_clicked() {
   ...
   if (response == QMessageBox::Ok) {
      ...
      QFile::copy(inout_path + "Inp_NOS3.txt",
                  inout_path + "__default__/Inp_NOS3.txt");
      receive_data();
      apply_data();
   } else {
      return;
...
void NOS_Menu::on_applyButton_clicked() {
   ...
   nos_update.append(whitespace(data_inp) +
                     " !  NOS3 Time Connection String\n");
