...
DSM_Menu::DSM_Menu(QWidget *parent) : QDialog(parent), ui(new Ui::DSM_Menu) {
   ui->setupUi(this);
...
void DSM_Menu::set_validators() {
   QRegularExpression noCmdSpacerRx(
       "[^" + QRegularExpression::escape(cmdDelimiter) + "]*");
   ...
   ui->cmdPvTgtType->addItems(
       dsm_gui_lib::sortStringList(cmdAttTgtTypes.values()));
   ui->cmdPvTgtAxisFrm->clear();
   ui->cmdPvTgtAxisFrm->addItems(
       dsm_gui_lib::sortStringList(cmdAttTgtFrm.values()));
   ui->cmdPvTgtWld->clear();
   ui->cmdPvTgtWld->addItems(
       dsm_gui_lib::sortStringList(dsm_gui_lib::worldInputs));

   ui->cmdSvTgtType->clear();
   ui->cmdSvTgtType->addItems(
       dsm_gui_lib::sortStringList(cmdAttTgtTypes.values()));
   ui->cmdSvTgtAxisFrm->clear();
   ui->cmdSvTgtAxisFrm->addItems(
       dsm_gui_lib::sortStringList(cmdAttTgtFrm.values()));
   ui->cmdSvTgtWld->clear();
   ui->cmdSvTgtWld->addItems(
       dsm_gui_lib::sortStringList(dsm_gui_lib::worldInputs));

   ui->cmdQuatFrm->clear();
   ui->cmdQuatFrm->addItems(
       dsm_gui_lib::sortStringList(cmdTrnFrmConst.values()));
   ...
   for (dsmSectionTypes type : section2CmdKeys) {
      ...
      if (newItem != NULL) {
         ...
         newItem->setData(cmdCols::cmdColLabel, cmdData::cmdType,
                          section2Cmd[type]);
...
void DSM_Menu::receive_data() {
   ...
   ui->cmdSC->clear();
   ui->cmdTrnOri->clear();
   ui->cmdTrnOri->addItems(
       dsm_gui_lib::sortStringList(cmdTrnOriConst.values()));
   ui->cmdTrnFrm->clear();
   ui->cmdTrnFrm->addItems(
       dsm_gui_lib::sortStringList(cmdTrnFrmConst.values()));
   ui->cmdPvTgtSc->clear();
   ui->cmdSvTgtSc->clear();
      ...
      if (line.contains("Spacecraft", Qt::CaseInsensitive)) {
         ...
         for (int i = 0; i < nSC; i++) {
            ...
            if (badScNames.contains(name)) {
               QString errMsg =
                   "Spacecraft name \"" + name + "\" is invalid.\n";
            ...
            nBdys.insert(
                name, dsm_gui_lib::get_sc_nitems(
                          inoutPath, name, dsm_gui_lib::scSectionType::BODY));
            nWhls.insert(
                name, dsm_gui_lib::get_sc_nitems(
                          inoutPath, name, dsm_gui_lib::scSectionType::WHEEL));
            nMtbs.insert(name,
                         dsm_gui_lib::get_sc_nitems(
                             inoutPath, name, dsm_gui_lib::scSectionType::MTB));
            nThrs.insert(name, dsm_gui_lib::get_sc_nitems(
                                   inoutPath, name,
                                   dsm_gui_lib::scSectionType::THRUSTER));
         }
         break;
      }
   }
   ...
   for (dsmSectionTypes type : searchOrd) {
      ...
      while (!in.atEnd()) {
         ...
         if (!match.captured(1).isEmpty()) {
            new_entry_item(type, match.captured(3), match.captured(1).toInt(),
                           match.captured(2));
         }
      }
   }
...
QStringList DSM_Menu::secDescription(const dsmSectionTypes type) {
   ...
   switch (type) {
      case dsmSectionTypes::COMMANDS:
         ...
         descrip.append("#-----------------------------------------------------"
                        "--------------------------");
         descrip.append("# Col: 1   ->  Command Interp ID Flag (hard coded in "
                        "interpreter)");
         descrip.append("# Col: 2   ->  Spacecraft ID Number: \"SC[#]\"");
         descrip.append("# Col: 3   ->  Command Sequence Flag: \"FswMode\" or "
                        "\"CmdTime\"");
         descrip.append("# Col: 4   ->  Command Sequence Parameter: FSW Mode "
                        "ID or Command Execution Time");
         descrip.append("# If Col: 3 -> CmdTime, then Col: 5 is \"NUM_CMD[#]\" "
                        "(\"#\" is the number of commands)");
         descrip.append("#                       Command Types:");
         descrip.append("#                       Translation Command "
                        "Method_Mode (\"PASSIVE_TRN\" for none)");
         descrip.append("#                       Attitude Command Method_Mode "
                        "(\"PASSIVE_ATT\" for none)");
         descrip.append(
      ...
      case dsmSectionTypes::TRANSLATION:
         ...
         descrip.append("# Col: 5   ->  Ref Origin (ctrl wrt): Orbit Point "
                        "\"OP\" or SC ID Number (\"SC[#].B[#]\")");
         descrip.append("# Col: 6   ->  Ref Frame Directions: \"N\", \"F\", "
                        "\"L\", or SC ID Number (\"SC[#].B[#]\")");
      ...
      case dsmSectionTypes::PRIMARY_VEC:
         ...
         descrip.append("# Col: 7   ->  Controller Mode");
         descrip.append("# Col: 8   ->  Actuator Mode");
         descrip.append("# If Col: 2 -> VEC, then Col: 6 is the Ref. Frame of "
                        "the pointing vector: \"N\", \"F\", \"L\"");
         descrip.append("#                        Col: 7-9 are the pointing "
                        "vec for Primary axis");
         descrip.append("#                        Col: 10-11 are Controller "
                        "and Actuator Sets");
         break;
      case dsmSectionTypes::SECONDARY_VEC:
         descrip.append("#------------------------------Secondary Axis "
                        "Cmd-------------------------------");
         descrip.append(
         ...
         descrip.append("# Col: 6   ->  Secondary Target: Specify Body, "
                        "SC[#].B[#] or Vec");
         descrip.append("# If Col: 2 -> VEC, then Col: 6 is the Ref. Frame of "
                        "the pointing vector: \"N\", \"F\", \"L\"");
         descrip.append("#                        Col: 7-9 are the pointing "
                        "vec for Secondary axis");
         break;
      case dsmSectionTypes::QUATERION:
         descrip.append("#------------------------Quaternion Attitude "
                        "Cmd--------------------------------");
         descrip.append(
      ...
      case dsmSectionTypes::MIRROR:
         descrip.append("#----------------------------Mirror Attitude "
                        "Cmd--------------------------------");
         descrip.append(
      ...
      case dsmSectionTypes::DETUMBLE:
         descrip.append("#---------------------------Detumble Attitude "
                        "Cmd-------------------------------");
         descrip.append(
      ...
      case dsmSectionTypes::WHLHMANAGEMENT:
         descrip.append("#---------------------------Wheel Momentum Management "
                        "Cmd-------------------------------");
         descrip.append(
             "# Col: 1   ->  Command Interp ID Flag: \"WhlHManageCmd_[#]\"");
         descrip.append("# Col: 2   ->  Momentum Dumping [ON/OFF]");
         descrip.append("# Col: 3   ->  Minimum H_norm [Nms] (0 to always "
                        "dump, *large* to never dump)");
         descrip.append("# Col: 4   ->  Maximum H_norm [Nms] (0 to always "
                        "dump, *large* to never dump)");
      ...
      case dsmSectionTypes::CONTROLLERS:
         ...
         descrip.append("# Col: 2   ->  Controller Type: \"PID_CNTRL\", "
                        "\"LYA_ATT_CNTRL\", \"LYA_2BODY_CNTRL\", "
                        "\"H_DUMP_CNTRL\", \"CUSTOM_CNTRL\"");
      ...
      case dsmSectionTypes::ACTUATORS:
         ...
         descrip.append("# Col: 2   ->  Actuators: \"WHL_[#]\", \"MTB\", "
                        "\"THR\", \"Ideal\"");
         break;
      case dsmSectionTypes::GAINS:
         ...
         descrip.append("# Col: 2   ->  Gain Mode: \"PID\", \"PID_WN\", "
                        "\"MomentumDump\", \"FC_LYA\", \"CUSTOM\"");
      ...
      case dsmSectionTypes::LIMITS:
         ...
         descrip.append("# Col: 2   ->  Max. Acceleration Direction 1, "
                        "frc_max/trq_max [N, Nm]");
         descrip.append("# Col: 3   ->  Max. Acceleration Direction 2, "
                        "frc_max/trq_max [N, Nm]");
         descrip.append("# Col: 4   ->  Max. Acceleration Direction 3, "
                        "frc_max/trq_max [N, Nm]");
         descrip.append("# Col: 5   ->  Max. Velocity Direction 1, "
                        "vel_max/w_max [m/s, deg/s]");
         descrip.append("# Col: 6   ->  Max. Velocity Direction 2, "
                        "vel_max/w_max [m/s, deg/s]");
         descrip.append("# Col: 7   ->  Max. Velocity Direction 3, "
                        "vel_max/w_max [m/s, deg/s]");
         break;
      case dsmSectionTypes::MANEUVER:
         ...
         descrip.append("# Col: 6   ->  Approximation Type [%s]: \"CONSTANT\", "
                        "\"SMOOTHED\". Note that");
         descrip.append("#    \"SMOOTHED\" Cmd must start at half of burn "
                        "duration before the equivalent DeltaV");
...
void DSM_Menu::new_entry_item(const dsmSectionTypes type, QString label,
                              const int itemNum, const QString itemData) {
   ...
   const static QRegularExpression rxCmdParse(
       "\\s*CmdTime\\s*([0-9.]+)\\s*NUM_CMD\\[[0-9]+]\\s*(.*)");
   ...
   switch (type) {
      case dsmSectionTypes::COMMANDS:
         ...
         dataSplit =
             match.captured(2).split(QRegExp("\\s"), Qt::SkipEmptyParts);
         if (itemNum >= scNames.length()) {
            dsm_gui_lib::error_message(
                "More Spacecraft in \"Inp_DSM.txt\" than in \"Inp_Sim.txt\".");
            dsm_gui_lib::inexplicable_error_message();
            DSM_Menu::close();
         ...
         newTreeItem->setData(tlCols::tlColTime, Qt::DisplayRole,
         ...
         for (const QString &cmd : qAsConst(dataSplit)) {
            ...
            if (cmdType != DSM_Menu::cmdPsvTrn &&
                cmdType != DSM_Menu::cmdPsvAtt) {
               label.clear();
               ...
               if (cmdType != cmdTypes::cmdAtt) {
                  ...
                  for (int i = 0; i < treeParentItem->childCount(); i++) {
                     ...
                     if (treeItem->data(cmdCols::cmdColInd, cmdData::cmdNum) ==
                         searchNum) {
                  ...
                  for (int subCmd : {cmdPV, cmdSV}) {
                     ...
                     for (int i = 0; i < treeParentItem->childCount(); i++) {
                        treeItem = treeParentItem->child(i);
                        if (treeItem->data(cmdCols::cmdColInd,
                                           cmdData::cmdNum) == searchNum) {
      ...
      case dsmSectionTypes::TRANSLATION:
         newTreeItem =
             new QTreeWidgetItem(entryCmdParents[section2Cmd[type]], {label});
         ...
         newTreeItem->setText(cmdCols::cmdColData,
                              dataSplit.join(cmdDataSpacer));
      ...
      case dsmSectionTypes::GAINS:
         ...
         newListItem->setData(gainsData::gainsData,
                              dataSplit.mid(1).join(cmdDataSpacer));
...
void DSM_Menu::set_command_ctrl_act(QTreeWidgetItem *item,
   ...
   if (match.captured(1).isEmpty()) {
      dsm_gui_lib::error_message(item->text(cmdCols::cmdColLabel) +
                                 R"( is missing "Controller_[#]".)");
      return;
   }
   searchNum = match.captured(1).toInt();
   QList<QTreeWidgetItem *> treeItems =
       ui->ctrlConfigTree->findItems("*", Qt::MatchWildcard);
   foreach (QTreeWidgetItem *treeItem, treeItems) {
      if (treeItem->data(ctrlCols::ctrlColInd, ctrlData::ctrlNum) ==
          searchNum) {
   ...
   if (match.captured(1).isEmpty()) {
      dsm_gui_lib::error_message(item->text(cmdCols::cmdColLabel) +
                                 R"( is missing "Actuator_[#]".)");
      return;
   }
   searchNum = match.captured(1).toInt();
   QList<QListWidgetItem *> listItems =
       ui->actList->findItems("*", Qt::MatchWildcard);
...
QString DSM_Menu::entryItemName(const dsmSectionTypes type) {
   switch (type) {
      case dsmSectionTypes::COMMANDS:
         return "DSM_Cmd  SC";
      case dsmSectionTypes::TRANSLATION:
         return "TranslationCmd_";
      case dsmSectionTypes::PRIMARY_VEC:
         return "AttitudeCmd_PV";
      case dsmSectionTypes::SECONDARY_VEC:
         return "AttitudeCmd_SV";
      case dsmSectionTypes::QUATERION:
         return "QuaternionCmd_";
      case dsmSectionTypes::MIRROR:
         return "MirrorCmd_";
      case dsmSectionTypes::DETUMBLE:
         return "DetumbleCmd_";
      case dsmSectionTypes::WHLHMANAGEMENT:
         return "WhlHManageCmd_";
      case dsmSectionTypes::ACTUATOR_CMD:
         return "ActuatorCmd_";
      case dsmSectionTypes::CONTROLLERS:
         return "Controller_";
...
int DSM_Menu::getCmdType(QString cmdString) {
   if (cmdString.contains("PASSIVE_TRN"))
      return cmdPsvTrn;
   else if (cmdString.contains("PASSIVE_ATT"))
      return cmdPsvAtt;
   else if (cmdString.contains("TranslationCmd_"))
      return cmdTrn;
      ...
      if (cmdRegEx(cmdAtt).match(cmdString).captured(2).isEmpty())
         ...
         return cmdAtt;
   } else if (cmdString.contains("AttitudeCmd_SV"))
      return cmdSV;
   else if (cmdString.contains("QuaternionCmd_"))
      return cmdQuat;
   else if (cmdString.contains("MirrorCmd_"))
      return cmdMirror;
   else if (cmdString.contains("DetumbleCmd_"))
      return cmdDetumble;
   else if (cmdString.contains("WhlHManageCmd"))
      return cmdWhlHManage;
   else if (cmdString.contains("ActuatorCmd_"))
      return cmdAct;
   else if (cmdString.contains("ManeuverCmd_"))
      return cmdManeuver;
   else
      return -1;
}

/* This function sets up the Regular Expression to parse lines from the
 * Inp_DSM.txt file */
QRegularExpression DSM_Menu::entryItemRegEx(const dsmSectionTypes type) {
   ...
   const static QRegularExpression rxCmd(
       format.arg(entryItemName(dsmSectionTypes::COMMANDS)
                      .split(QRegExp("\\s"), Qt::SkipEmptyParts)
                      .join("\\s*")));
   const static QRegularExpression rxTrn(
       format.arg(entryItemName(dsmSectionTypes::TRANSLATION)));
   const static QRegularExpression rxPV(
       format.arg(entryItemName(dsmSectionTypes::PRIMARY_VEC)));
   const static QRegularExpression rxSV(
       format.arg(entryItemName(dsmSectionTypes::SECONDARY_VEC)));
   const static QRegularExpression rxQuat(
       format.arg(entryItemName(dsmSectionTypes::QUATERION)));
   const static QRegularExpression rxMirror(
       format.arg(entryItemName(dsmSectionTypes::MIRROR)));
   const static QRegularExpression rxDetumble(
       format.arg(entryItemName(dsmSectionTypes::DETUMBLE)));
   const static QRegularExpression rxWhlHManage(
       format.arg(entryItemName(dsmSectionTypes::WHLHMANAGEMENT)));
   const static QRegularExpression rxActCmd(
       format.arg(entryItemName(dsmSectionTypes::ACTUATOR_CMD)));
   const static QRegularExpression rxCtrl(
       format.arg(entryItemName(dsmSectionTypes::CONTROLLERS)));
   const static QRegularExpression rxAct(
       format.arg(entryItemName(dsmSectionTypes::ACTUATORS)));
   const static QRegularExpression rxGains(
       format.arg(entryItemName(dsmSectionTypes::GAINS)));
   const static QRegularExpression rxLims(
       format.arg(entryItemName(dsmSectionTypes::LIMITS)));
   const static QRegularExpression rxManeuver(
       format.arg(entryItemName(dsmSectionTypes::MANEUVER)));
   ...
   switch (type) {
      ...
      case dsmSectionTypes::MANEUVER:
         return rxManeuver;
      default:
         return QRegularExpression(format.arg(entryItemName(type)));
   }
}
...
QRegularExpression DSM_Menu::cmdRegEx(const int cmdType) {
   ...
   const static QRegularExpression rxTrn(
       format.arg(entryItemName(dsmSectionTypes::TRANSLATION)));
   const static QRegularExpression rxAtt(
       format.arg(entryItemName(dsmSectionTypes::PRIMARY_VEC)) +
       format.arg("_SV"));
   const static QRegularExpression rxPV(
       format.arg(entryItemName(dsmSectionTypes::PRIMARY_VEC)));
   ...
   switch (cmdType) {
      ...
      default:
         return QRegularExpression(
             format.arg(entryItemName(section2Cmd.key(cmdType))));
   }
}
...
void DSM_Menu::cmd_data_changed() {
   ...
   int cmdType =
       curItem->parent()->data(cmdCols::cmdColLabel, cmdData::cmdType).toInt();
   switch (cmdType) {
      case cmdTrn:
         ...
         if (checkList.contains(tmpStr))
            ...
            cmdData.append(scBdyFrmt.arg(scNames.indexOf(tmpStr))
                               .arg(ui->cmdTrnOriScBdyNum->value()));
         }
         ...
         if (checkList.contains(tmpStr))
            ...
            cmdData.append(scBdyFrmt.arg(scNames.indexOf(tmpStr))
                               .arg(ui->cmdTrnFrmScBdyNum->value()));
         }
         break;
      case cmdPV:
         ...
         if (tmpStr.compare("VEC") == 0) {
            cmdData.append(
                cmdAttTgtFrm.key(ui->cmdPvTgtAxisFrm->currentText(), "N"));
      ...
      case cmdSV:
         ...
         if (tmpStr.compare("VEC") == 0) {
            cmdData.append(
                cmdAttTgtFrm.key(ui->cmdSvTgtAxisFrm->currentText(), "N"));
      ...
      case cmdQuat:
         ...
         checkSum = 0.0;
         for (int i = 0; i < 4; i++)
            checkSum += cmdData[i].toDouble();
         ui->cmdQuatNormalize->setEnabled(
             checkSum > std::numeric_limits<double>::epsilon());
      ...
      case cmdMirror:
         ...
         cmdData.append(scBdyFrmt.arg(scNames.indexOf(tmpStr))
                            .arg(ui->cmdMirrorTgtBdyNum->value()));
         break;
      case cmdDetumble:
         // Nothing here
         break;
      ...
      case cmdManeuver:
         ...
         cmdData.append(
             cmdManTypes.key(ui->cmdManType->currentText(), "SMOOTHED"));
   ...
   if (cmdType == cmdAct) {
      QList<QTreeWidgetItem *> checkItems =
          ui->cmdTimelineTree->findItems(curItem->text(cmdCols::cmdColLabel),
                                         Qt::MatchExactly, tlCols::tlColAct);
...
void DSM_Menu::on_cmdLabel_textEdited(const QString &arg1) {
   ...
   QList<QTreeWidgetItem *> changeItems =
       ui->cmdTimelineTree->findItems("*", Qt::MatchWildcard, checkCol);
   static QRegularExpression oldKeyRx;

   if (cmdType == cmdPV)
      oldKeyRx = QRegularExpression(
          "^(?(?=" +
          QRegularExpression::escape(oldKey + cmdDelimiter + cmdDataSpacer) +
          ".*)" + QRegularExpression::escape(oldKey) +
          "(?=" + QRegularExpression::escape(cmdDelimiter + cmdDataSpacer) +
          ")|" + QRegularExpression::escape(oldKey) + ")");
   else if (cmdType == cmdSV)
      oldKeyRx = QRegularExpression(
          QRegularExpression::escape(cmdDelimiter + cmdDataSpacer + oldKey) +
          "$");
...
void DSM_Menu::on_cmdConfigTree_currentItemChanged(QTreeWidgetItem *item,
                                                   QTreeWidgetItem *) {
   ...
   if (isNull) {
      ui->cmdConfigurator->setTabVisible(ui->cmdConfigurator->currentIndex(),
                                         false);
      ui->cmdLabel->clear();
      return;
   }
   ...
   dsm_gui_lib::setQComboBox(ui->cmdController,
                             item->text(DSM_Menu::cmdColCtl));
   ...
   switch (cmdType) {
      case cmdTrn:
         ...
         if (cmdDataSplit[3].contains("SC")) {
            ...
            dsm_gui_lib::setQComboBox(ui->cmdTrnOri,
                                      cmdTrnOriConst[cmdDataSplit[3]]);
         ...
         if (cmdDataSplit[4].contains("SC")) {
            ...
            dsm_gui_lib::setQComboBox(ui->cmdTrnFrm,
                                      cmdTrnFrmConst[cmdDataSplit[4]]);
         }
         break;
      case cmdPV:
         dsm_gui_lib::setQComboBox(ui->cmdPvTgtType,
                                   cmdAttTgtTypes[cmdDataSplit[0]]);
         ...
         if (cmdDataSplit[0].compare("VEC") == 0) {
            dsm_gui_lib::setQComboBox(ui->cmdPvTgtAxisFrm,
                                      cmdAttTgtFrm[cmdDataSplit[4]]);
      ...
      case cmdSV:
         ...
         dsm_gui_lib::setQComboBox(ui->cmdSvTgtType,
                                   cmdAttTgtTypes[cmdDataSplit[0]]);
         ...
         if (cmdDataSplit[0].compare("VEC") == 0) {
            dsm_gui_lib::setQComboBox(ui->cmdSvTgtAxisFrm,
                                      cmdAttTgtFrm[cmdDataSplit[4]]);
      ...
      case cmdQuat:
         ...
         dsm_gui_lib::setQComboBox(ui->cmdQuatFrm,
                                   cmdTrnFrmConst[cmdDataSplit[4]]);
         break;
      case cmdMirror:
      ...
      case cmdDetumble:
         ...
         break;
      case cmdWhlHManage:
         ui->cmdHManageEnabled->setChecked(
             cmdDataSplit[0].compare("ON", Qt::CaseInsensitive) == 0);
      ...
      case cmdManeuver:
         ...
         dsm_gui_lib::setQComboBox(ui->cmdManType,
                                   cmdManTypes[cmdDataSplit[4]]);
...
void DSM_Menu::on_loadDefaultButton_clicked() {
   int response = dsm_gui_lib::warning_message(
       "Delete Inp_DSM file and load current Default?");
...
void DSM_Menu::on_saveDefaultButton_clicked() {
   int response = dsm_gui_lib::warning_message(
       "Delete Default Inp_DSM file and replace with current?");
   if (response == QMessageBox::Ok) {
...
void DSM_Menu::on_cmdRemove_clicked() {
   ...
   const QString warnMsg =
       QString("Command \"%1\" is used by spacecraft \"%2\" at time %3 "
               "seconds%5.\n\n") +
   ...
   if (!searchList.isEmpty()) {
      ...
      QString cmdTime =
          searchList[0]->data(tlCols::tlColTime, Qt::DisplayRole).toString();
      ...
      int response         = dsm_gui_lib::warning_message(warnMsg.arg(
          label, scName, cmdTime, cmdTypeStr, andElsewhere, nTimes, isPlural));
   ...
   if (cmdType == cmdPV) {
      if (cmdPV == cmdType &&
          ui->cmdAttLabel->currentIndex() == ui->cmdAttLabel->findText(label)) {
...
void DSM_Menu::on_cmdAdd_clicked() {
   ...
   for (int i = 0; i < newCmdTypeParent->childCount(); i++)
      usedCmdInds.append(newCmdTypeParent->child(i)
                             ->data(cmdCols::cmdColInd, cmdData::cmdNum)
                             .toInt());

   int newCmdInd;
   for (newCmdInd = 0; newCmdInd < 5e3; newCmdInd++)
      if (!usedCmdInds.contains(newCmdInd))
         break;
   ...
   if (cmdType == cmdTrn) {
      if (cmdValidCtrls[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Translation Controller.");
         return;
      }
      tmpCtrlItems =
          ui->ctrlConfigTree->findItems(cmdValidCtrls[cmdType].first(),
                                        Qt::MatchExactly, cmdCols::cmdColLabel);
      ctrlName = entryItemFormat(dsmSectionTypes::CONTROLLERS)
                     .arg(tmpCtrlItems.first()
                              ->data(cmdCols::cmdColInd, cmdData::cmdNum)
                              .toInt());
   } else if (attCmds.contains(cmdType)) {
      if (cmdValidCtrls[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Attitude Controller.");
         return;
      }
      tmpCtrlItems =
          ui->ctrlConfigTree->findItems(cmdValidCtrls[cmdType].first(),
      ...
      if (cmdValidCtrls[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Momentum Management Controller.");
         return;
      }
      tmpCtrlItems =
          ui->ctrlConfigTree->findItems(cmdValidCtrls[cmdType].first(),
   ...
   if (trnCmds.contains(cmdType)) {
      if (cmdValidActs[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Translation Actuator.");
         return;
      }
      tmpActItems = ui->actList->findItems(cmdValidActs[cmdType].first(),
                                           Qt::MatchExactly);
      actName     = entryItemFormat(dsmSectionTypes::ACTUATORS)
                    .arg(tmpActItems.first()->data(actData::actNum).toInt());
   } else if (attCmds.contains(cmdType)) {
      if (cmdValidActs[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Attitude Actuator.");
         return;
      }
      tmpActItems = ui->actList->findItems(cmdValidActs[cmdType].first(),
                                           Qt::MatchExactly);
      ...
      if (cmdValidActs[cmdType].isEmpty()) {
         dsm_gui_lib::error_message(
             "You must first define a valid Momentum Management Actuator.");
         return;
      }
      tmpActItems = ui->actList->findItems(cmdValidActs[cmdType].first(),
                                           Qt::MatchExactly);
   ...
   switch (type) {
      ...
      case dsmSectionTypes::MANEUVER:
         ...
         limName =
             entryItemFormat(dsmSectionTypes::LIMITS)
                 .arg(ui->limList->item(0)->data(limData::limNum).toInt());
   ...
   new_entry_item(type, newLabel, newCmdInd,
                  newCmdData.split(QRegExp("//s"), Qt::SkipEmptyParts)
                      .join(cmdDataSpacer));
   populate_cmdtl_dropdowns(cmdType);
}
...
void DSM_Menu::on_cmdDuplicate_clicked() {
   ...
   for (int i = 0; i < curItem->parent()->childCount(); i++) {
      ...
      usedCmdInds.append(curItem->parent()
                             ->child(i)
                             ->data(cmdCols::cmdColInd, cmdData::cmdNum)
                             .toInt());
   }
   ...
   for (int i = 0; i < 5e3; i++) {
      ...
      if (i > 0)
         newLabelTest += "_" + QString::number(i);
...
void DSM_Menu::on_cmdActList_currentItemChanged(QListWidgetItem *current,
                                                QListWidgetItem *) {
   if (current == NULL)
      return;
...
void DSM_Menu::on_cmdTimelineTree_currentItemChanged(QTreeWidgetItem *current,
                                                     QTreeWidgetItem *) {
   ...
   if (current->text(tlCols::tlColTrn).isEmpty())
      ...
      dsm_gui_lib::setQComboBox(ui->cmdTrnLabel,
                                current->text(tlCols::tlColTrn));

   if (current->text(tlCols::tlColAct).isEmpty())
      dsm_gui_lib::setQComboBox(ui->cmdActLabel, "No Change");
   else
      dsm_gui_lib::setQComboBox(ui->cmdActLabel,
                                current->text(tlCols::tlColAct));

   if (current->text(tlCols::tlColHMan).isEmpty())
      dsm_gui_lib::setQComboBox(ui->cmdHManLabel, "No Change");
   else {
      dsm_gui_lib::setQComboBox(ui->cmdHManLabel,
                                current->text(tlCols::tlColHMan));
   }
   ...
   ui->cmdTime->setValue(
       current->data(tlCols::tlColTime, Qt::DisplayRole).toDouble());
}
...
void DSM_Menu::timeline_data_changed() {
   ...
   if (attCmd.compare("No Change") == 0) {
      ...
      if (attCmdsHash[attCmd].contains(
              entryItemName(dsmSectionTypes::PRIMARY_VEC))) {
...
void DSM_Menu::validate_sv_cmds(QString curAttCmd) {
   ...
   QString cmdPVData;
   QList<QTreeWidgetItem *> items = ui->cmdConfigTree->findItems(
       pvLabel, Qt::MatchExactly | Qt::MatchRecursive, cmdCols::cmdColLabel);
   for (QTreeWidgetItem *item : qAsConst(items)) {
      if (item->parent() != NULL &&
          item->parent()
                  ->data(cmdCols::cmdColLabel, cmdData::cmdType)
                  .toInt() == cmdTypes::cmdPV) {
   ...
   QVector3D cmdPVAxis(dataSplit[1].toDouble(), dataSplit[2].toDouble(),
                       dataSplit[3].toDouble());
   ...
   for (int i = 0; i < entryCmdParents[cmdSV]->childCount(); i++) {
      ...
      QStringList svDataSplit = svCmd->text(cmdCols::cmdColData)
                                    .split(QRegExp("\\s"), Qt::SkipEmptyParts);
      QString svType = svDataSplit[0];
      QVector3D svAxis(svDataSplit[1].toDouble(), svDataSplit[2].toDouble(),
                       svDataSplit[3].toDouble());
      QString svTgtType = svDataSplit[4];

      QVector3D crossAxis = QVector3D::crossProduct(
          cmdPVAxis / cmdPVAxis.length(), svAxis / svAxis.length());

      double smallCheck = std::numeric_limits<double>::epsilon();

      if (crossAxis.length() <= smallCheck)
         svIsValid = false;

      if (isVEC && (svType.compare("VEC") == 0)) {
         QVector3D svTgtAxis(svDataSplit[5].toDouble(),
                             svDataSplit[6].toDouble(),
                             svDataSplit[7].toDouble());
         if (cmdPVTgtThing.compare(svTgtType) == 0) {
            crossAxis =
                QVector3D::crossProduct(svTgtAxis / svTgtAxis.length(),
                                        cmdPVTgtAxis / cmdPVTgtAxis.length());
            if (crossAxis.length() <= smallCheck)
               svIsValid = false;
         }

      } else if (cmdPVTgtThing.compare(svTgtType) == 0)
         svIsValid = false;

      if (svIsValid)
         validCmdSV.append(svCmd->text(cmdCols::cmdColLabel));
   }
   ui->cmdAttSVLabel->clear();
...
void DSM_Menu::on_actList_currentItemChanged(QListWidgetItem *current,
                                             QListWidgetItem *) {
   bool isNull = current == NULL;
   ...
   dsm_gui_lib::setQComboBox(
       ui->actType, actTypes[current->data(actData::actType).toString()]);
...
void DSM_Menu::on_actType_textActivated(const QString &arg1) {
   ...
   QList<QTreeWidgetItem *> items = ui->cmdConfigTree->findItems(
       label, Qt::MatchExactly | Qt::MatchRecursive, cmdCols::cmdColAct);
   for (QTreeWidgetItem *item : items)
      on_cmdConfigTree_itemChanged(item,
                                   cmdCols::cmdColAct); // revalid actuator type
}
...
void DSM_Menu::on_cmdConfigTree_itemChanged(QTreeWidgetItem *item, int column) {
   ...
   switch (column) {
      case cmdCols::cmdColAct:
         ...
         if (actValidCmds[checkListItem->data(actData::actType).toString()]
                 .contains(cmdType))
      ...
      case cmdCols::cmdColCtl:
         checkTreeItem =
             ui->ctrlConfigTree
                 ->findItems(text, Qt::MatchExactly, ctrlCols::ctrlColLabel)
                 .at(0);
         if (ctrlValidCmds[ctrlTypes.key(
                               checkTreeItem->text(ctrlCols::ctrlColType))]
                 .contains(cmdType))
            item->setBackground(column, okTextBrush);
         else
            item->setBackground(column, badTextBrush);
...
void DSM_Menu::on_actAdd_clicked() {
   ...
   for (newActInd = 0; newActInd < 5e3; newActInd++) {
      ...
      if (!usedCmdInds.contains(newActInd) && !usedLabels.contains(newLabel))
         break;
   }
...
void DSM_Menu::on_actDuplicate_clicked() {
   ...
   int newActInd;
   for (newActInd = 0; newActInd < 5e3; newActInd++)
      if (!usedCmdInds.contains(newActInd))
         break;
   ...
   new_entry_item(dsmSectionTypes::ACTUATORS, newLabel, newActInd,
                  curItem->data(actData::actType).toString());
...
void DSM_Menu::populate_cmdtl_dropdowns(int cmdType) {
   ...
   if (trnCmds.contains(cmdType)) {
      ...
      ui->cmdAttSVLabel->clear();
      bool isPVCmd = attCmdsHash[pvLabel].contains(
          entryItemName(dsmSectionTypes::PRIMARY_VEC));
      ui->cmdAttSVLabel->setEnabled(isPVCmd);
      if (isPVCmd)
         validate_sv_cmds(curAttCmd);
   } else if (cmdType == cmdWhlHManage) {
...
void DSM_Menu::populate_cmd_dropdowns() {
   ...
   ui->cmdController->clear();
   if (curCmd != NULL && curCmd->parent() != NULL) {
      int cmd = curCmd->parent()
                    ->data(cmdCols::cmdColLabel, cmdData::cmdType)
                    .toInt();
      ui->cmdActuator->addItems(dsm_gui_lib::sortStringList(cmdValidActs[cmd]));
      ui->cmdController->addItems(
          dsm_gui_lib::sortStringList(cmdValidCtrls[cmd]));
      dsm_gui_lib::setQComboBox(ui->cmdActuator,
                                curCmd->text(cmdCols::cmdColAct));
      dsm_gui_lib::setQComboBox(ui->cmdController,
                                curCmd->text(cmdCols::cmdColCtl));
   }
}

void DSM_Menu::on_ctrlConfigTree_currentItemChanged(QTreeWidgetItem *current,
                                                    QTreeWidgetItem *) {
   bool isNull = current == NULL;
   ...
   dsm_gui_lib::setQComboBox(ui->ctrlType,
                             current->text(ctrlCols::ctrlColType));
   ui->ctrlLabel->setText(current->text(ctrlCols::ctrlColLabel));
   dsm_gui_lib::setQComboBox(ui->ctrlLims,
                             current->text(ctrlCols::ctrlColLims));

   QString ctrlType = ctrlTypes.key(current->text(ctrlCols::ctrlColType));

   ui->ctrlGains->clear();
   ui->ctrlGains->addItems(
       dsm_gui_lib::sortStringList(ctrlValidGains[ctrlType]));
   dsm_gui_lib::setQComboBox(ui->ctrlGains,
                             current->text(ctrlCols::ctrlColGains));
}

void DSM_Menu::on_ctrlConfigTree_itemChanged(QTreeWidgetItem *item,
                                             int column) {
   if (item == NULL)
      return;
   QString text = item->text(column);
   ...
   switch (column) {
      case ctrlCols::ctrlColGains:
         checkItem =
             ui->gainList->findItems(item->text(column), Qt::MatchExactly)
                 .at(0);
         if (allowableGains[ctrlType].contains(
                 checkItem->data(gainsData::gainsType).toString()))
            item->setBackground(column, okTextBrush);
         else
            item->setBackground(column, badTextBrush);
         break;
...
void DSM_Menu::on_ctrlType_textActivated(const QString &arg1) {
   ...
   QList<QTreeWidgetItem *> items = ui->cmdConfigTree->findItems(
       label, Qt::MatchExactly | Qt::MatchRecursive, cmdCols::cmdColCtl);
   for (QTreeWidgetItem *item : items)
      on_cmdConfigTree_itemChanged(
          item, cmdCols::cmdColCtl); // revalid controller type

   ui->ctrlGains->clear();
   ui->ctrlGains->addItems(
       dsm_gui_lib::sortStringList(ctrlValidGains[newType]));
   dsm_gui_lib::setQComboBox(ui->ctrlGains,
                             current->text(ctrlCols::ctrlColGains));
...
void DSM_Menu::on_ctrlAdd_clicked() {
   ...
   for (int i = 0; i < ui->ctrlConfigTree->topLevelItemCount(); i++) {
      usedCmdInds.append(ui->ctrlConfigTree->topLevelItem(i)
                             ->data(ctrlCols::ctrlColInd, ctrlData::ctrlNum)
                             .toInt());
      usedLabels.append(
          ui->ctrlConfigTree->topLevelItem(i)->text(cmdCols::cmdColLabel));
   }
   ...
   for (newInd = 0; newInd < 5e3; newInd++) {
      ...
      if (!usedCmdInds.contains(newInd) && !usedLabels.contains(newLabel))
         break;
   }
   ...
   for (QString testNewType : ctrlValidGains.keys()) {
      if (!ctrlValidGains[testNewType].isEmpty()) {
         ...
         gains   = entryItemFormat(dsmSectionTypes::GAINS)
                     .arg(ui->gainList->findItems(gains, Qt::MatchExactly)
                              .at(0)
                              ->data(gainsData::gainsNum)
                              .toInt());
         break;
      }
   }
   ...
   if (ui->limList->count() == 0) {
      dsm_gui_lib::error_message(
          "You must first define a valid set of Limits.");
      return;
   }

   QString lims = entryItemFormat(dsmSectionTypes::LIMITS)
                      .arg(ui->limList->item(0)->data(limData::limNum).toInt());

   new_entry_item(dsmSectionTypes::CONTROLLERS, newLabel, newInd,
                  newType + cmdDataSpacer + gains + cmdDataSpacer + lims);
   populate_cmd_dropdowns();
}
...
void DSM_Menu::on_ctrlDuplicate_clicked() {
   ...
   for (int i = 0; i < ui->ctrlConfigTree->topLevelItemCount(); i++) {
      usedCmdInds.append(ui->ctrlConfigTree->topLevelItem(i)
                             ...
                             .toInt());
      usedLabels.append(
          ui->ctrlConfigTree->topLevelItem(i)->text(ctrlCols::ctrlColLabel));
   }

   int newInd;
   for (newInd = 0; newInd < 5e3; newInd++)
      if (!usedCmdInds.contains(newInd))
         break;
   ...
   lims         = entryItemFormat(dsmSectionTypes::LIMITS)
              .arg(ui->limList->findItems(lims, Qt::MatchExactly)
                       .at(0)
                       ->data(limData::limNum)
                       .toInt());
...
void DSM_Menu::on_gainList_currentItemChanged(QListWidgetItem *current,
                                              QListWidgetItem *) {
   bool isNull = current == NULL;
   ...
   for (int i = 0; i < ui->gainConfigurator->count(); i++) {
      ...
      ui->gainConfigurator->setTabVisible(
          i, gainType.compare(gainsTypes.key(tabText)) == 0);
   }

   dsm_gui_lib::setQComboBox(ui->gainType, gainsTypes[gainType]);

   QStringList dataSplit = current->data(gainsData::gainsData)
                               .toString()
                               .split(QRegExp("\\s"), Qt::SkipEmptyParts);
...
void DSM_Menu::gain_data_changed() {
   ...
   } else if (current->data(gainsData::gainsType)
                  .toString()
                  .compare("PID_WN") == 0) {
...
void DSM_Menu::on_gainType_textActivated(const QString &arg1) {
   ...
   if (currentCtrl != NULL) {
      ctrlValidGainType = allowableGains[ctrlTypes.key(
          currentCtrl->text(ctrlCols::ctrlColType))];
   ...
   QList<QTreeWidgetItem *> items = ui->ctrlConfigTree->findItems(
       label, Qt::MatchExactly, ctrlCols::ctrlColGains);
   for (QTreeWidgetItem *item : items)
...
void DSM_Menu::on_gainLabel_textEdited(const QString &arg1) {
   ...
   QList<QTreeWidgetItem *> items = ui->ctrlConfigTree->findItems(
       oldKey, Qt::MatchExactly, ctrlCols::ctrlColGains);
   for (QTreeWidgetItem *item : items)
...
void DSM_Menu::on_gainAdd_clicked() {
   ...
   for (newInd = 0; newInd < 5e3; newInd++) {
      ...
      if (!usedInds.contains(newInd) && !usedLabels.contains(newLabel))
         break;
   }
   ...
   new_entry_item(dsmSectionTypes::GAINS, newLabel, newInd,
                  data.join(cmdDataSpacer));
...
void DSM_Menu::on_gainDuplicate_clicked() {
   ...
   QString data =
       current->data(gainsData::gainsType).toString() + cmdDataSpacer;
...
void DSM_Menu::populate_ctrl_dropdowns() {
   ...
   if (curCtrl != NULL) {
      ui->ctrlGains->clear();
      QStringList validGains =
          ctrlValidGains[ctrlTypes.key(curCtrl->text(ctrlCols::ctrlColType))];
      validGains.sort();
      ui->ctrlGains->addItems(validGains);
      dsm_gui_lib::setQComboBox(ui->ctrlGains,
                                curCtrl->text(ctrlCols::ctrlColGains));
      dsm_gui_lib::setQComboBox(ui->ctrlLims,
                                curCtrl->text(ctrlCols::ctrlColLims));
   }
}

void DSM_Menu::on_limList_currentItemChanged(QListWidgetItem *current,
                                             QListWidgetItem *) {
   bool isNull = current == NULL;
   ...
   QStringList dataSplit = current->data(limData::limData)
                               .toString()
                               .split(QRegExp("\\s"), Qt::SkipEmptyParts);
...
void DSM_Menu::on_limLabel_textEdited(const QString &arg1) {
   ...
   current->setText(arg1);
   QList<QTreeWidgetItem *> items = ui->ctrlConfigTree->findItems(
       oldKey, Qt::MatchExactly | Qt::MatchRecursive, ctrlCols::ctrlColLims);
   for (QTreeWidgetItem *item : items)
...
void DSM_Menu::on_limRemove_clicked() {
   ...
   QString label                  = current->text();
   QList<QTreeWidgetItem *> items = ui->ctrlConfigTree->findItems(
       label, Qt::MatchExactly | Qt::MatchRecursive, ctrlCols::ctrlColLims);
   for (QTreeWidgetItem *item : qAsConst(items)) {
...
void DSM_Menu::on_limAdd_clicked() {
   ...
   new_entry_item(dsmSectionTypes::LIMITS, newLabel, newInd,
                  data.join(cmdDataSpacer));
...
void DSM_Menu::on_limDuplicate_clicked() {
   ...
   new_entry_item(dsmSectionTypes::LIMITS, newLabel, newInd,
                  current->data(limData::limData).toString());
   populate_ctrl_dropdowns();
}
...
void DSM_Menu::on_applyButton_clicked() {
   ...
   for (auto i = searchOrd.crbegin(); i != searchOrd.crend(); i++) {
      ...
      switch (type) {
         case dsmSectionTypes::COMMANDS:
            ui->cmdTimelineTree->sortByColumn(tlCols::tlColSC,
                                              Qt::AscendingOrder);
            ui->cmdTimelineTree->sortByColumn(tlCols::tlColTime,
                                              Qt::AscendingOrder);
            treeItems = ui->cmdTimelineTree->findItems("*", Qt::MatchWildcard,
                                                       tlCols::tlColSC);
            ...
            for (QTreeWidgetItem *cmdItem : qAsConst(treeItems)) {
               ...
               time =
                   cmdItem->data(tlCols::tlColTime, Qt::DisplayRole).toDouble();
               dataList.append(
                   label.arg(scNames.indexOf(cmdItem->text(tlCols::tlColSC)))
                       .arg(time, 15, 'f', 3));
               ...
               if (!cmdItem->text(tlCols::tlColAtt).isEmpty()) {
                  ...
                  tmp.clear();
                  data =
                      rxPV.match(cmdItem->text(tlCols::tlColAtt)).captured(1);
                  tmp.append(attCmdsHash[data]);
                  data =
                      rxSV.match(cmdItem->text(tlCols::tlColAtt)).captured(1);
                  if (!data.isEmpty()) {
                     checkItems = ui->cmdConfigTree->findItems(
                         data, Qt::MatchExactly | Qt::MatchRecursive,
                         cmdCols::cmdColLabel);
            ...
            break;
         case dsmSectionTypes::CONTROLLERS:
            ui->ctrlConfigTree->sortByColumn(ctrlCols::ctrlColInd,
                                             Qt::AscendingOrder);
            treeItems = ui->ctrlConfigTree->findItems("*", Qt::MatchWildcard,
                                                      ctrlCols::ctrlColLabel);
            ...
            for (QTreeWidgetItem *ctrlItem : qAsConst(treeItems)) {
               ...
               dataList.append(
                   ctrlTypes.key(ctrlItem->text(ctrlCols::ctrlColType)));

               data = ctrlItem->text(ctrlCols::ctrlColGains);
               if (data.isEmpty()) {
                  dsm_gui_lib::error_message(
                      "Gains are missing for Controller: " + label);
                  dsmUpdate.clear();
                  return;
               }
               if (ctrlItem->background(ctrlCols::ctrlColGains) ==
                   badTextBrush) {
                  dsm_gui_lib::error_message("Controller \"" + label +
                                             "\" has invalid gains: \"" + data +
                                             "\"");
                  dsmUpdate.clear();
                  return;
               }
               ...
               dataList.append(
                   entryItemFormat(dsmSectionTypes::GAINS)
                       .arg(listItem->data(gainsData::gainsNum).toInt()));

               data = ctrlItem->text(ctrlCols::ctrlColLims);
               if (data.isEmpty()) {
                  dsm_gui_lib::error_message(
                      "Limits are missing for Controller: " + label);
                  dsmUpdate.clear();
                  return;
               }
               if (ctrlItem->background(ctrlCols::ctrlColLims) ==
                   badTextBrush) {
                  dsm_gui_lib::error_message("Controller \"" + label +
                                             "\" has invalid Limits: \"" +
                                             data + "\"");
                  dsmUpdate.clear();
                  // This really shouldn't happen except in the case of missing
                  // text
                  dsm_gui_lib::inexplicable_error_message();
                  return;
               }
               ...
               dataList.append(
                   entryItemFormat(dsmSectionTypes::LIMITS)
                       .arg(listItem->data(limData::limNum).toInt()));
            ...
            ui->ctrlConfigTree->sortByColumn(ctrlCols::ctrlColLabel,
                                             Qt::AscendingOrder);
         ...
         default: // handle commands
            ...
            for (int i = 0; i < parent->childCount(); i++) {
               ...
               if (treeItem->background(cmdCols::cmdColCtl) == badTextBrush) {
                  dsm_gui_lib::error_message(tmp + "Command \"" + label +
                                             "\" has invalid Controller: \"" +
                                             data + "\"");
                  dsmUpdate.clear();
                  return;
               }
               if (!data.isEmpty())
                  dataList.append(
                      metaHash[dsmSectionTypes::CONTROLLERS]->value(data));

               data = treeItem->text(cmdCols::cmdColAct);
               if (treeItem->background(cmdCols::cmdColAct) == badTextBrush) {
                  dsm_gui_lib::error_message("Command \"" + label +
                                             "\" has invalid Actuator: \"" +
                                             data + "\"");
                  dsmUpdate.clear();
                  return;
               }
               if (!data.isEmpty())
                  dataList.append(
                      metaHash[dsmSectionTypes::ACTUATORS]->value(data));
...
bool DSM_Menu::check_actuator_cmds(const QString scName,
                                   const QTreeWidgetItem *actCmd) {
   const static QRegularExpression actCmdRx(
       "[a-zA-Z]{3}_\\[([0-9]+)]_\\[[0-9.]+]");
   ...
   QStringList dataSplit = actCmd->text(cmdCols::cmdColData)
                               .split(QRegExp("\\s"), Qt::SkipEmptyParts);
   ...
   bool isErr     = false;
   QString errMsg = "Actuator command \"" + label +
                    "\" requires more actuators than spacecraft \"" + scName +
                    "\" has:\n";
   if (nUsedWhl > nWhl) {
      isErr = true;
      errMsg.append(tmpStr.arg(QString("Wheel:   "), QString::number(nUsedWhl),
                               scName, QString::number(nWhl)));
   }
   if (nUsedMtb > nMtb) {
      isErr = true;
      errMsg.append(tmpStr.arg(QString("MTB:     "), QString::number(nUsedMtb),
                               scName, QString::number(nMtb)));
   }
   if (nUsedThr > nThr) {
      isErr = true;
      errMsg.append(tmpStr.arg(QString("Thruster:"), QString::number(nUsedThr),
                               scName, QString::number(nThr)));
   }
   if (isErr) {
      dsm_gui_lib::error_message(errMsg);
   }
